...
- `includes/admin/` - settings page under **Settings** (`class-ace-ads-manager-admin.php` + `views/settings.php`): tabs →
  fieldset sections → fields from the settings schema; custom tabs render via `ace_ads_settings_tab_content`;
  `class-ace-ads-manager-guide.php` holds the manual (Guide tab + WP help tabs + per-tab guide panels).
