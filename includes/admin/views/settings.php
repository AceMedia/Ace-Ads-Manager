<?php
/**
 * Settings page view for Ace Ads Manager.
 *
 * Variables: $options, $fields, $tabs, $intro, $admin (see Ace_Ads_Manager_Admin::render_settings_page).
...
?>
<div class="wrap ace-redis-settings ace-ads-manager-settings">
    <h1>
        <span class="dashicons dashicons-admin-generic ace-page-icon" aria-hidden="true"></span>
        <?php echo esc_html( get_admin_page_title() ); ?>
        <span class="ace-version">v<?php echo esc_html( ACE_ADS_VERSION ); ?></span>
    </h1>
    <?php if ( ! empty( $intro ) ) : ?>
                ...
                <?php foreach ( $tabs as $index => $tab ) : ?>
                    ...
                    <?php elseif ( 'guide' === $tab['id'] ) : ?>
                        <div class="ace-tab-subnav" data-tab="guide">
                            <?php foreach ( Ace_Ads_Manager_Guide::sections() as $gid => $gsection ) : ?>
                    ...
                    <?php endif; ?>
                <?php endforeach; ?>
                <?php do_action( 'ace_ads_settings_sidebar' ); ?>
            </nav>
        </div>
        ...
        <div class="ace-redis-content">
            ...
            <form id="ace-redis-settings-form" method="post" class="ace-redis-form">
                <?php foreach ( $field_tabs as $tab ) : ?>
                    <div id="<?php echo esc_attr( $tab['id'] ); ?>" class="tab-content<?php echo $tab['id'] === $first_tab ? ' active' : ''; ?>">
                        ...
                        <?php Ace_Ads_Manager_Admin::guide_panel( $tab['id'], $tab['help'] ?? '' ); ?>
                        <?php do_action( 'ace_ads_settings_tab_before', $tab['id'] ); ?>
                        <div class="settings-form">
                            <?php foreach ( (array) ( $tab['sections'] ?? [] ) as $section ) : ?>
                                <fieldset id="<?php echo esc_attr( $section['id'] ); ?>" class="ace-settings-group">
                                    ...
                                    <?php endforeach; ?>
                                    <?php do_action( 'ace_ads_settings_section_after', $section['id'], $tab['id'] ); ?>
                                </fieldset>
                            <?php endforeach; ?>
                        </div>
                        <?php do_action( 'ace_ads_settings_tab_after', $tab['id'] ); ?>
                    </div>
                <?php endforeach; ?>
            </form>
            ...
            <?php foreach ( $custom_tabs as $tab ) : ?>
                <div id="<?php echo esc_attr( $tab['id'] ); ?>" class="tab-content ace-custom-tab<?php echo $tab['id'] === $first_tab ? ' active' : ''; ?>">
                    ...
                    <?php Ace_Ads_Manager_Admin::guide_panel( $tab['id'], $tab['help'] ?? '' ); ?>
                    <?php if ( 'guide' === $tab['id'] ) : ?>
                        <?php Ace_Ads_Manager_Guide::render(); ?>
                    <?php else : ?>
                        <?php do_action( 'ace_ads_settings_tab_content', $tab['id'] ); ?>
                    <?php endif; ?>
                </div>
            <?php endforeach; ?>
        </div>
