 ...
 *
 * Variables: $options, $fields, $tabs, $admin (see Ace_Ads_Manager_Admin::render_settings_page).
...
<div class="wrap ace-redis-settings ace-ads-manager-settings">
    <h1><?php echo esc_html( get_admin_page_title() ); ?> <span class="ace-version">v<?php echo esc_html( ACE_ADS_VERSION ); ?></span></h1>
    ...
    <div class="ace-redis-container">
        <div class="ace-redis-sidebar">
            <nav class="nav-tab-wrapper">
                 ...
                 */
                do_action( 'ace_ads_settings_sidebar' );
                ?>
            </nav>
        </div>
            ...
            <form id="ace-redis-settings-form" method="post" class="ace-redis-form">
                <?php foreach ( $tabs as $index => $tab ) : list( $tab_id, $tab_label ) = $tab; ?>
                    <div id="<?php echo esc_attr( $tab_id ); ?>" class="tab-content<?php echo 0 === $index ? ' active' : ''; ?>">
                        ...
                        <?php do_action( 'ace_ads_settings_tab_before', $tab_id ); ?>
                        <table class="form-table" role="presentation">
                            <?php foreach ( $fields as $key => $field ) : if ( ( $field['tab'] ?? $first_tab ) !== $tab_id ) { continue; } ?>
                                <tr>
                                    <th scope="row"><label for="<?php echo esc_attr( Ace_Ads_Manager_Admin::PAGE . '-' . $key ); ?>"><?php echo esc_html( $field['label'] ); ?></label></th>
                                    <td><?php $admin->render_field( $key, $field, $options[ $key ] ?? null ); ?></td>
                                </tr>
                            <?php endforeach; ?>
                        </table>
                        <?php do_action( 'ace_ads_settings_tab_after', $tab_id ); ?>
                    </div>
                <?php endforeach; ?>
            </form>
        </div>
